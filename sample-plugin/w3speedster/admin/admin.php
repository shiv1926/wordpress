...
<main class="admin-speedster">
	...
	<li class="active w3_general"><a data-toggle="tab" href="#general"><?php _e( 'General', 'w3speedster' ); ?></a></li>
		...
		<div class="icon_container"> <span class="h-icon"><img src="<?php echo W3SPEEDSTER_PLUGIN_URL;?>assets/images/css-h.png"></span></div>
				...
				<tr>
					...
					<tr>
						...
						?>							 
						<td><button type="button" id="create_critical_css" <?php echo (empty($result['load_critical_css'])) ? 'disabled="true"' : ''; ?>><?php _e( 'Create Critical Css Now', 'w3speedster' ); ?></button></td>
						...
						<td><span class="preload_error_css"><?php echo (empty($result['load_critical_css'])) ? __( '*Please enable load critical css and save to start generating critical css', 'w3speedster' ) : w3_get_option('w3speedup_critical_css_error');?></span></td>
						<style>
						#create_critical_css{
							margin-bottom:25px;
						}
							...
							function create_critical_css(){
								...
								jQuery.ajax({
									...
									success:function(data) {
										data = jQuery.parseJSON( data );
										console.log(data);
										if(data[0] == 'success' || (data[0] == 'error' && data[1] == 'process already running')){
					...
					<tr>
						<th scope="row"><?php _e( 'Load Secondary Css', 'w3speedster' ); ?><span class="info"></span><span class="info-display"><?php _e( 'Choose when to load secondary css(which are not preloaded)', 'w3speedster' ); ?>.</span></th>
						<td><select name="load_combined_css">
							<option value="on_page_load" <?php echo !empty($result['load_combined_css']) && $result['load_combined_css'] == 'on_page_load' ? 'selected' : '' ;?>><?php _e( 'On Page Load', 'w3speedster' ); ?></option>
							<option value="after_page_load" <?php echo !empty($result['load_combined_css']) && $result['load_combined_css'] == 'after_page_load' ? 'selected' : '' ;?>><?php _e( 'After Page Load', 'w3speedster' ); ?></option>
							</select>
						</td>
					</tr>
					<tr>
					<th scope="row"><?php _e( 'Delay secondary css by', 'w3speedster' ); ?><span class="info"></span><span class="info-display"><?php _e( 'Enter in seconds to delay loading of secondary css', 'w3speedster' ); ?>.</span></th>
						<td>
					...
					<tr>
						...
						<td><textarea name="custom_css" rows="10" cols="16" placeholder="<?php _e( 'Please Enter css without the style tag.', 'w3speedster' ); ?>" ><?php if (!empty($result['custom_css'])) echo $result['custom_css'];?></textarea>
						</td>
					</tr>
					<tr>
			...
			<table class="form-table">
				<tbody>
					...
					<tr>
						...
						<td class="top"><?php _e( 'Defer', 'w3speedster' ); ?>&nbsp;<input type="checkbox" name="exclude_js_defer" <?php if (!empty($result['exclude_js_defer']) && $result['exclude_js_defer'] == "on") echo "checked";?> ></td>
					</tr>
					<tr>
					...
					<tr>
					<th scope="row"><?php _e( 'Delay Internal Javascript Tags by', 'w3speedster' ); ?><span class="info"></span><span class="info-display"><?php _e( 'Enter in seconds to delay loading of combined javascript', 'w3speedster' ); ?></span></th>
						<td>
					...
					<?php /*<tr>
					<th scope="row"><?php _e( 'Delay External Javascript Tags by', 'w3speedster' ); ?><span class="info"></span><span class="info-display"><?php _e( 'Enter in seconds to delay loading of external javascript', 'w3speedster' ); ?></span></th>
						<td>
...
<div class="icon_container"> <span class="h-icon"><img src="<?php echo W3SPEEDSTER_PLUGIN_URL;?>assets/images/import-export-icon.png"></span></div>
	...
	<section id="opt_img1" class="tab-pane fade">
		<form method="post">
			<table class="form-table">
				<tbody>
					<tr>
					...
					if(!empty($combine_images)){
						...
						?>
							<tr class="image_src_field">
								<td style="width:70%; padding-left:0px;"><input type="text" name="combine_images[<?php echo $i; ?>][src]" placeholder="Please Enter Img Src" value="<?php echo $value['src'] ;?>"></td>
								<td style="padding-left:0px;"><input type="text" name="combine_images[<?php echo $i; ?>][position]" placeholder="Please Enter Image Width" value="<?php echo $value['position'] ;?>"></td>
						...
						<tr class="image_src_field">
							<td style="width:70%; padding-left:0px;"><input type="text" name="combine_images[0][src]" placeholder="Please Enter Img Src" value=""></td>
							<td style="padding-left:0px;"><input type="text" name="combine_images[0][position]" placeholder="Please Enter Image Width" value=""></td>
							<td class="remove_image_field" style="width:5%; cursor:pointer;">X</td>
						</tr>
					<?php } ?>
					...
					<?php $combine_image_att_id = w3_get_option( 'w3_speedup_combine_image_id');
