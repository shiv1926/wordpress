...
class w3speedster_css extends w3speedster{
	...
	function w3_convert_arr_relative_to_absolute($string, $url, $matches){
		...
		foreach($matches as $match){
			...
			if(substr($match1, 0, 1) == '/' || strpos($match1,'http') !== false){
				$match1 = $this->add_settings['wp_site_url'].'/'.trim($match_arr['path'],'/');
				$import_match = $match1;
			}else{
			...
			if(strpos($match1,'.css')!== false && strpos($string,'@import '.$match)!== false && $url != $this->add_settings['wp_home_url'].'/index.php'){
                $string = str_replace('@import '.$match.';',$this->w3_relative_to_absolute_path($import_match,file_get_contents(str_replace($this->add_settings['wp_site_url'],$this->add_settings['wp_document_root'],$import_match))), $string);
                continue;
			}
			...
			$imgsrc_filepath = str_replace($img_root_url,'',$this->add_settings['home_url'].$match_arr['path']);
			...
			if(!empty($this->settings['webp_png']) && in_array($ext, $webp_enable)){
    ...
    function minify_css($html, $css_links){
		...
		if(!empty($css_links) && !empty($this->settings['css'])){
			...
			foreach($css_links as $key => $css){
				...
				if( (!empty($css_obj['rel']) && $css_obj['rel'] == 'stylesheet' && !empty($css_obj['href'])) || empty($css_obj['rel']) ){
					...
					$css_links_arr[] = array('arr'=>array(),'css'=>'');
				}
			}
				...
				if(!empty($css_obj['rel']) && $css_obj['rel'] == 'stylesheet' && !empty($css_obj['href'])){
					...
					if($exclude_css1){
						if($enable_cdn && $this->w3_endswith($css_obj['href'], '.css')){
							$this->w3_str_replace_set_css($css,str_replace($this->add_settings['wp_site_url'],$this->add_settings['image_home_url'],$css));
						}
						continue;
					}
					$force_lazy_load = 0;
					...
					if($force_lazy_load){
						...
						if(!empty($final_merge_css[$combined_css_file_counter]) && is_array($final_merge_css[$combined_css_file_counter])){
							...
							$this->w3_str_replace_set_css($final_merge_css[$combined_css_file_counter][count($final_merge_css[$combined_css_file_counter])-1]['css'],'{{'.$combined_css_file.'}}');
			...
			$all_inline_css = (!empty($this->settings['custom_css']) ? $this->w3_css_compress($this->settings['custom_css']) : '').'@keyframes fadeIn {  to {    opacity: 1;  }}.fade-in {  opacity: 0;  animation: fadeIn .5s ease-in 1 forwards;}.is-paused {  animation-play-state: paused;}';
				$html = $this->w3_insert_content_head($html,'<style id="w3speedster-custom-css">'.$all_inline_css.'</style>',4);
		...
		$html = $this->w3_insert_content_head($html , '<script>var w3_googlefont='.json_encode($google_font).';</script>',3);
		return $html;
	}
	function w3_generate_combined_css($final_merge_css,$enable_cdn){
		if(is_array($final_merge_css) && count($final_merge_css) > 0){
			...
			if(!empty($file_name)){
				...
				if($enable_cdn){
					$secondary_css = str_replace($this->add_settings['wp_site_url'],$this->add_settings['image_home_url'],$secondary_css);
				}
